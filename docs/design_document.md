...
load_bundle(model_meta) → check session cache → if miss: download from HuggingFace
    │
    ▼
render_schema_form(schema) → {field_name: value} dict
    │
    ▼
...
bundle.model.predict(X) → scalar float
    │
    ▼
PredictionResult(value, model_id, target, warnings, raw_input, feature_vector)
```
...
| `selectbox` | `st.selectbox` | `values` list |
