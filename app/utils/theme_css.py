...
def inject_theme_css() -> None:
    ...
    css = (
        ...
        # Slider
        #
        # The filled/empty track gradient is applied by react-range as an
        # INLINE background: linear-gradient(...) on the InnerTrack element.
        # Inline styles cannot be overridden by any CSS class rule regardless
        # of !important. The filled color (primary) and empty color
        # (borderOpaque, derived from page background) are driven by the
        # BaseWeb theme object Streamlit builds from config.toml at page load.
        # config_writer.py handles this when running locally by writing
        # primaryColor and backgroundColor into config.toml on every theme
        # switch so the gradient is correct on the next rerun.
        # On Streamlit Cloud the CSS layer below keeps all visible elements
        # themed per-session without touching any shared file.
        #
        # What CSS controls:
        #   - Thumb circle color and border
        #   - Thumb focus ring
        #   - Thumb value text (floating number above thumb)
        #   - Tick bar text (min/max labels)
        # ------------------------------------------------------------------

        # Thumb circle -- filled with primary color
        ...
        "    box-shadow: 0 0 0 4px rgba(" + primary_rgb + ", 0.30) !important;\n"
        "    outline: none !important;\n"
        "}\n"

        # Thumb value text -- the floating current-value label above the thumb
        "[data-testid='stSliderThumbValue'] {\n"
        "    color: " + text_main + " !important;\n"
        "}\n"

        # Tick bar -- min/max boundary labels beneath the track
