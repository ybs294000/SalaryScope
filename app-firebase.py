...
# ==================================================
def extract_salary_a1(consequents):
...
def get_assoc_insight_a1(education, experience, country, job_group, predicted_salary, rules):
    ...
    if country != "Other":
        for _, row in rules.iterrows():
            ...
            if (
                ...
                return (
                    f"{experience}-level professionals in {job_group} roles in {country} "
                    f"are commonly associated with {predicted_salary} salary ranges "
    ...
    if best_rule is not None:
        salary = extract_salary_a1(best_rule["consequents"])
        return (
            f"{experience}-level professionals in {job_group} roles "
            f"are most commonly associated with {salary} salary ranges "
            f"(Confidence: {round(best_conf*100, 2)}%)."
        )
    ...
    for _, row in rules.iterrows():
        ...
        if f"Experience_Category_{experience}" in ant:
            salary = extract_salary_a1(row["consequents"])
            return (
                f"{experience}-level professionals are generally associated with "
                f"{salary} salary ranges."
            )
...
- SQLite-backed user, session, and prediction database
    ...
    st.markdown("""
- Prediction history stored per logged-in user (model, inputs, salary, timestamp)
...
- Predictions saved to database for logged-in users
    """)
